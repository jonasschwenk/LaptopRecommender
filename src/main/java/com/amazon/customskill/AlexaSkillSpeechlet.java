...
//	private static int[] answers = new int[6];
	...
	public SpeechletResponse onIntent(SpeechletRequestEnvelope<IntentRequest> requestEnvelope) {
		...
		if (lastQuestion.equalsIgnoreCase("Wie viel möchtest du maximal ausgeben?")) {
			...
			if (budget < minimumPrice)
				return askUserResponse(
						"Dein Budget liegt unterhalb des Preises meiner Minimalkonfiguration! Daher werde ich dir keinen Laptop innerhalb deines Budegets empfehlen können. Möchtest du die Beratung dennoch Fortsetzen?");
		...
		if (counter != 0 && lastQuestion.equals("")) {
			...
			return askUserResponse("Diese Aufgabe hätte nicht kommen dürfen!");
	}
